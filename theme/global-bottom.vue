...
function usePloy(number = 16) {
  function getPoints(): Range[] {
    ...
    return Array.from({ length: number })
      .fill(0)
      .map(() => [applyOverflow(randomBetween(limits.x), overflow), applyOverflow(randomBetween(limits.y), overflow)]);
  }
...
<template>
  <div>
    <div
      class="transform-gpu overflow-hidden pointer-events-none theme-bg"
      :style="{ filter: `blur(70px) hue-rotate(${hue}deg)` }"
      :class="[theme === 'light' ? 'bg-white scale-150' : 'bg-black']"
      aria-hidden="true"
    >
      <template>
        <div
          class="clip bg-linear-to-r from-(--slidev-theme-from) to-(--slidev-theme-to) theme-bg-polygon"
          :style="{ 'clip-path': `polygon(${poly1})`, opacity: opacity }"
        />
        <div
          class="clip bg-linear-to-l from-(--slidev-theme-from) to-(--slidev-theme-to) theme-bg-polygon"
          :style="{ 'clip-path': `polygon(${poly2})`, opacity: opacity }"
        />
        <div
          class="clip bg-gradient-to-l from-[#312a5c] to-[#416387] theme-bg-polygon"
